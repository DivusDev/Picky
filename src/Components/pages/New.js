...
class New extends Component {
    ...
    getPosts = () => {
        ...
        fetch("https://ipapi.co/json/").then((data) => data.json()).then(res => {this.setState({ ip:res.ip, city:res.city, region:res.region })
            ...
            let posts =  postsDB.orderBy("posted", "desc").limit(15).get().then(snapshot => {
                let array = [];
        ...
        return (
            <div>
                {this.state.posts.map((e) => {
                    return <Post key={e.id} id={e.id} city={e.city} region={e.region} liked={e.liked} caption={e.caption} url={e.url} flashes={e.flashes}/>
                })}
